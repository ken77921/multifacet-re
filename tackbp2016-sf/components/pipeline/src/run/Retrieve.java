...
import indexir.AccentInsensitiveAnalyzer;
...
public class Retrieve {
  ...
  public static void main(String[] args) throws IOException, ParseException {
    ...
    String stopWordsFn = args[4];
    boolean strict = args.length >= 6 && args[5].equals("true");

    QueryList ql = new QueryList(queryXmlFn);

    BufferedReader br = new BufferedReader(new FileReader(stopWordsFn));
	...
	for (QueryList.Query q : ql.getQueries()) {                                                                                                                           
		...
		if(!delimAlias.contains(" " + q.getName() + " ")){                                                                  
		    queryStr += " \"" + QueryParser.escape(alias) + "\"^0.01";                                                                                                  
		}                                                                                                                                                              
	    }                                                                                                                                                                   
	    query = parser.parse(queryStr);                                                                                                                                    
	    hits = is.search(query, numResults);
