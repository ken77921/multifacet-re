...
from model import model as model_code
...
#We could also use this class to put all sentences with the same length together
    def __init__(self, feature, target, kb_marker, num_basis, device):
        self.feature = feature
        self.target = target
        self.kb_marker = kb_marker
    ...
    def __getitem__(self, idx):
        ...
        if self.kb_marker is None:
            kb_marker = []
        else:
            kb_marker = torch.tensor(self.kb_marker[idx], dtype = torch.bool, device = self.output_device)
        if self.num_basis is None:
            num_basis = []
        else:
            num_basis = torch.tensor(self.num_basis[idx], dtype = torch.long, device = self.output_device)

        #debug target[-1] = idx
        return [feature, target, kb_marker, num_basis]
        #return [self.feature[idx, :], self.target[idx, :]]

def create_data_loader_split(f_in, bsz, device, split_num, copy_training):
    feature_target_kb = torch.load(f_in, map_location='cpu')
    if len(feature_target_kb) == 4:
        feature, target, kb_marker, num_basis = feature_target_kb
    elif len(feature_target_kb) == 3:
        feature, target, kb_marker = feature_target_kb
        num_basis = None
    else:
        feature, target = feature_target_kb
        kb_marker, num_basis = None, None
    ...
    if copy_training:
        ...
        for i in range(split_num):
            ...
            dataset_arr.append(  F2SetDataset(feature[start:end], \
                                              target[start:end], \
                                              kb_marker if kb_marker is None else kb_marker[start:end], \
                                              num_basis if num_basis is None else num_basis[start:end], \
                                              device ) ) #assume that the dataset are randomly shuffled beforehand
        #dataset_arr = [ F2SetDataset(feature[idx_arr[i,:],:], target[idx_arr[i,:],:], device) for i in range(split_num)]
    else:
        dataset_arr = [ F2SetDataset(feature[i:feature.size(0):split_num,:], \
                                     target[i:target.size(0):split_num,:], \
                                     kb_marker if kb_marker is None else kb_marker[i:kb_marker.size(0):split_num], \
                                     num_basis if num_basis is None else num_basis[i:num_basis.size(0):split_num], \
                                     device) \
                        for i in range(split_num) ]
    ...
    if device.type == 'cuda':
        use_cuda = True
    dataloader_arr = [torch.utils.data.DataLoader(dataset_arr[i], batch_size = bsz, shuffle = True, pin_memory=not use_cuda, drop_last=(len(dataset_arr[i]) % bsz == 1)) for i in range(split_num)]
...
def create_data_loader(f_in, bsz, device, want_to_shuffle = True):
    ...
    #print(feature)
    #print(target)
    dataset = F2SetDataset(feature, target, kb_marker, num_basis, device)
    #dataset = F2SetDataset(feature[0:feature.size(0):2,:], target[0:target.size(0):2,:], device)
    use_cuda = False
    if device.type == 'cuda':
        use_cuda = True
    # pin_memory should be False - VERIFY
    drop_last = (len(dataset) % bsz == 1)
    return torch.utils.data.DataLoader(dataset, batch_size = bsz, shuffle = want_to_shuffle, pin_memory=not use_cuda, drop_last=drop_last)
...
def load_corpus(data_path, train_bsz, eval_bsz, device, tensor_folder = "tensors", training_file = "train.pt", split_num = 1, copy_training = False, skip_training = False, want_to_shuffle_val = True, skip_val = False):

    assert not (skip_training and skip_val), "Both training and validation can't be skipped!"

    train_corpus_name = os.path.join(data_path, tensor_folder, training_file)
    val_org_corpus_name = os.path.join(data_path, tensor_folder, "val_org.pt")
    val_shuffled_corpus_name = os.path.join(data_path, tensor_folder, "val_shuffled.pt")
    dictionary_input_name = os.path.join(data_path, "pattern_dictionary_index")
    target_dictionary_input_name = os.path.join(data_path, "entpair_dictionary_index")

    with open(dictionary_input_name) as f_in:
        idx2word_freq = load_idx2word_freq(f_in)
    
    with open(target_dictionary_input_name) as f_in:
        target_idx2word_freq = load_idx2word_freq(f_in)

    dataloader_val = None
    dataloader_val_shuffled = None
    if not skip_val:
        ...
        max_sent_len = dataloader_val_shuffled.dataset.feature.size(1)
    ...
    if skip_training:
        ...
        if not skip_val:
            ...
            max_sent_len = max_sent_len_train

    return idx2word_freq, target_idx2word_freq, dataloader_train_arr, dataloader_val, dataloader_val_shuffled, max_sent_len
...
def seed_all_randomness(seed,use_cuda,randomness=True):
    random.seed(seed)
    np.random.seed(seed)
    ...
    if torch.cuda.is_available():
        if not use_cuda:
            ...
            if not randomness:
                print("NOT using any randomness")
                torch.backends.cudnn.deterministic = True
                torch.backends.cudnn.benchmark = False
            else:
                print("Using randomness")
...
def save_checkpoint(encoder, decoder, optimizer_e,  optimizer_d, source_emb, target_emb, path):
    torch.save(encoder.state_dict(), os.path.join(path, 'encoder.pt'))
    try:
        torch.save(decoder.state_dict(), os.path.join(path, 'decoder.pt'))
    except:
        pass
    if source_emb.size(0) > 1:
        torch.save(source_emb, os.path.join(path, 'source_emb.pt'))
    if target_emb.size(0) > 1:
        torch.save(target_emb, os.path.join(path, 'target_emb.pt'))
